function [emean, ediv] = simple_model_fun(sys, meas)
lf = 0.4;
lr = 0.4;
lw = 0.3;
...
% odometer_error = 0; % percent (max error)
% odometer_noise = 0; % rad/sec
% gps_pos_error = 0; % meters (max error)
% gps_pos_noise = 0; % meters
% gps_vel_error = 0; % m/s (max error)
% gps_vel_noise = 0; % m/s
% gyro_bias = normrnd(0, 0.001); % rad/s
% gyro_noise = 0; % rad/s
...
%     end
    ...
    kalman_state.pos = sys(1);
    kalman_state.h = sys(2);
    kalman_state.v = sys(3);
    kalman_state.dh = sys(4);
    kalman_state.odo = sys(5);
%     kalman_state.U = [2e-5 2e-5 1e-5 1e-2 1e-3 8e-9 8e-9 8e-9 8e-9]';
    % init sensors struct
    sensors.gps_error_pos = meas(1);
    sensors.gps_error_vel = meas(2);
    sensors.gyro_error = meas(3);
    sensors.odo_error = meas(4);
    
%     sensors.gps_error_pos = 1e-9;
%     sensors.gps_error_vel = 1e-9;
%     sensors.gyro_error = 1e-9;
...
% figure
% plot(Time, Errors(:,1), 'LineWidth', 2);
% grid on
% hold on
ediv = std(Errors(:,1));
emean = mean(Errors(:,1));
% plot([Time(1) Time(end)], [mean(Errors(:,1)) mean(Errors(:,1))], 'k', 'LineWidth', 1.0);
% plot([Time(1) Time(end)], [mean(Errors(:,1))+shift mean(Errors(:,1))+shift], '--k', 'LineWidth', 1.0);
% plot([Time(1) Time(end)], [mean(Errors(:,1))-shift mean(Errors(:,1))-shift], '--k', 'LineWidth', 1.0);
