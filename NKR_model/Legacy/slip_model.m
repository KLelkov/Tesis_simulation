...
kalman_state.X = zeros(10,1);
kalman_state.X(3) = -pi/2; % set initial heading
% kalman_state.X(10) = 1;
kalman_state.P = diag([100 100 10 10 10 100 100 10 10 1]);
...
for i = 1:nSim
    ...
    betta(i) = Anr(i)/25; % assume that betta is quasi-constant
    ...
    dxb = V(i)*cos(betta(i));
    dyb = V(i)*sin(betta(i));
    alfa(i,1) = gamma(i,5) - atan( (dyb + lf*Anr(i)) / (dxb + lw*Anr(i)) );
    alfa(i,2) = gamma(i,5) - atan( (dyb + lf*Anr(i)) / (dxb - lw*Anr(i)) );
    %% Measurements model
    % Odometer errors
...
%     kalman_state = forwardNav(kalman_state, sensors, update);
    kalman_state = slipNav(kalman_state, sensors, update);
...
%     % sol key: [X Y Heading V dHeading w1 w2 w3 w4]
    ...
    Errors(i,5) = sqrt( (gps_vel(i,1) - sol(i,4)*cos(sol(i,3)))^2 + (gps_vel(i,2) - sol(i,4)*sin(sol(i,3)))^2);
    Errors(i,5) = - Errors(i,5)^2 + (gps_vel(i,1)^2 + gps_vel(i,2)^2) + sol(i,4)^2;
    Errors(i,5) = Errors(i,5) / (2*sqrt(gps_vel(i,1)^2 + gps_vel(i,2)^2)*sol(i,4));
    Errors(i,5) = gps_vel(i,1) / cos(sol(i,3));
    Errors(i,6) = gps_vel(i,1) / sin(sol(i,3));
%     if abs(sol(i,3)) > 0.3
%         Errors(i,5) = cot(Heading(i)) - (sensors.gps_dx - Anr(i)*(lsx*cos(Heading(i)) - lsy*sin(Heading(i)))) / (V(i) * sin(Heading(i)));
%     else
%         Errors(i,5) = (sensors.gps_dy - Anr(i)*(lsx*sin(Heading(i)) + lsy*cos(Heading(i)))) / (V(i) * cos(Heading(i))) - tan(Heading(i));
%     end
    %% Animation
    if Animation_enable == 1
...
plot([Time(1) Time(end)], [mean(Errors(:,1))+3*shift mean(Errors(:,1))+3*shift], '--k', 'LineWidth', 1.5);
plot([Time(1) Time(end)], [mean(Errors(:,1))-3*shift mean(Errors(:,1))-3*shift], '--k', 'LineWidth', 1.5);
